...
function failureReason(error) {
  ...
  if (message.includes('ssl')||message.includes('certificate')||message.includes('no pg_hba.conf entry')) return 'SSL_REQUIRED';
  return 'UNKNOWN';
}

export async function databaseStatus(env=process.env) {
  if (!env.DATABASE_URL) return {status:'MISSING',reason:null,ssl:'NOT CHECKED'};
  let url;
  try {
