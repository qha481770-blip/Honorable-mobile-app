...
async function fixture(t){
  ...
  const server=http.createServer(createMarketingApp({dataDir:dir,publicBaseUrl:'http://127.0.0.1',credentials:{}}));
  await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));const base=`http://127.0.0.1:${server.address().port}`;
  t.after(async()=>{await new Promise(resolve=>server.close(resolve));await rm(dir,{recursive:true,force:true});});return base;
}
...
test('website and dashboard are served with restrictive security headers',async t=>{
  ...
  const dashboard=await request(base,'/dashboard');assert.equal(dashboard.status,200);assert.match(dashboard.text(),/MOCK DATA/);
});
