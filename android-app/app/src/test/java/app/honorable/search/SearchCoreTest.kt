...
class SearchCoreTest {
    ...
    @Test fun `index compatibility detects preprocessing change`() { val current=IndexCompatibility();assertTrue(current.compatibleWith(current.copy()));assertFalse(current.compatibleWith(current.copy(preprocessingVersion="old")));assertEquals(4,current.schemaVersion) }
